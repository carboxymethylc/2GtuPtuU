...
NSString* NSStringFromKernel(CGFloat* kernel, size_t width, size_t height) 
{
    ...
    NSMutableString* matrix = [[NSMutableString alloc] autorelease];
	CGFloat sum = 0.f;
...
- (UIImage*)greyImage 
{
	CGImageRep* image = [[[CGImageRep alloc] initWithCGImage:[self CGImage]] autorelease];
	...
	CFMutableDataRep* destDateRep = [[[CFMutableDataRep alloc] 
							  ...
							  allocator:kCFAllocatorDefault] autorelease];
	...
	CGColorSpaceRef genericColorSpace = CGColorSpaceCreateDeviceGray();
...
- (UIImage*)hueImage:(UIColorDegrees)colorDegree hue:(CGFloat)hue 
	...
	CGImageMutableRep* image = [[[CGImageMutableRep alloc] 
										initWithCGImage:[self CGImage]] autorelease];
	size_t height = image.height;
	size_t width = image.width;
