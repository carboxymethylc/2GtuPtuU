...
//  GLImageProcessing
//
//  Created by Chirag Sunshine on 29/07/11.
...
- (void)constrainCropToImage {
    ...
    //cropView.frame = CGRectMake(0, 0,40, 40);
...
- (void)updateBounds {
    ...
    if(frame.size.width==selfWidth && frame.size.height>=selfHeight)
    {
        frame.size.height=self.imageView.frame.size.height;
    }
    if(frame.size.height==selfHeight && frame.size.width>=selfWidth)
    {
    ...
    if(cropView.frame.size.width<selfWidth && cropView.frame.size.height <selfHeight)
    {
...
+ (UIView *)initialCropView {
    ...
    //cropView.contentMode=UIViewContentModeScaleAspectFit;
    cropView.layer.borderColor = [[UIColor whiteColor] CGColor];
    cropView.layer.borderWidth = 2.0;
    
    //cropView.image = [UIImage imageNamed:@"frame.png"];
    cropView.backgroundColor = [UIColor clearColor];
    //cropView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"frame.png"]];
...
- (void)setup
{
    ...
    cropView = [ImageCropper initialCropView];
...
- (id)initWithImage:(UIImage*)newImage andMaxSize:(CGSize)maxSize {
    ...
    if (self)
    {
        ...
        self.frame = CGRectMake(0, 0, maxSize.width, maxSize.height);
        NSLog(@"\n  self.frame = height = %f, width = %f ", self.frame.size.height,self.frame.size.width);
        
        //imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE, IMAGE_CROPPER_OUTSIDE_STILL_TOUCHABLE)];
        
        
        //Chirag:image view.
        //imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0,280,333)];
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,0,maxSize.width,maxSize.height)];
        
        
        //imageView.contentMode = UIViewContentModeScaleAspectFill;
        
         NSLog(@"\n  imageView = height = %f, width = %f ", imageView.frame.size.height,imageView.frame.size.width);
...
- (UIImage*) getCroppedImage
{
    ...
    */
    ...
    CGRect drawRect = CGRectMake(-rect.origin.x, -rect.origin.y,320,320);
   

        NSLog(@"\n drawrect x= %f",drawRect.origin.x);
        NSLog(@"\n drawrect y= %f",drawRect.origin.y);
        NSLog(@"\n drawrect w= %f",drawRect.size.width);
        NSLog(@"\n drawrect h= %f",drawRect.size.height);
    
    // clip to the bounds of the image context
    // not strictly necessary as it will get clipped anyway?
    CGContextClipToRect(context, CGRectMake(0,0, rect.size.width, rect.size.height));
    
    NSLog(@"\n rect x= %f",rect.origin.x);
    NSLog(@"\n rect y= %f",rect.origin.y);
    NSLog(@"\n rect w= %f",rect.size.width);
    NSLog(@"\n rect h= %f",rect.size.height);
